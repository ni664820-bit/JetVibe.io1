...
if (heroPlane && heroSection) {
  ...
  function updatePlane() {
    ...
    const rotate = -t * 8;
    const scale = 1 - t * 0.3;
    const opacity = t < 0.05 ? Math.min(t / 0.05, 1) : (t > 0.9 ? Math.max((1 - t) / 0.1, 0) : 1);

    heroPlane.style.transform = `translate(${x}px, ${y}vh) rotate(${rotate}deg) scale(${scale})`;
